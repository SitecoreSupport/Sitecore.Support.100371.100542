namespace Sitecore.Support.ExperienceAnalytics.Api.Http.ModelBinding
{
    ...
    internal class ReportQueryModelBinder : IModelBinder
    {
        ...
        private readonly ISegmentDefinitionService segmentDefinitionService;
        private readonly IDecoder<string> textDecoder;

        public ReportQueryModelBinder()
            : this(Config.KeysTopDefault, ApiContainer.GetKeyCodec(), ApiContainer.GetLogger(), ApiContainer.Repositories.GetSegmentDefinitionService())
        {
        }

        public ReportQueryModelBinder(int keyTopDefaultValue, IDecoder<string> textDecoder, ILogger logger, ISegmentDefinitionService segmentDefinitionService)
        {
            if (textDecoder == null)
            {
